...
- [x] **PDP landscape videos — Tutorial + per-product Real footage (Shopify theme)** — Done + deployed 2026-06-12. Replaced the portrait `pdp-action` demo with two 16:9 sections: `pdp-tutorial` (shared video via section setting) and `pdp-real-footage` (per-product video from the new `santai.real_video` URL metafield; section hides when empty). Copy: "Easier than mascara." / "See it on a real eye." New PDP order: `main, comparison, tutorial, howto, suit, real_footage, ugc, reviews, editorial`. Metafield definition created + pinned on store. Pushed to draft theme #156386722014 (old `pdp-action.liquid` removed remotely too). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-12-pdp-landscape-videos*`. ⚠️ **Merchant still uploads the videos**: tutorial in the theme editor (PDP → "PDP — Tutorial"); real footage per product under Products → [product] → Metafields → "Real footage video".
